from socket import socket, AF_INET, SOCK_DGRAM; from datetime import datetime, timezone
...
# Pulled and slightly modified from the python version of the Rich Presence U plugin
# Github repo at https://github.com/FlamingNineteen/RichPresenceWUPS/blob/main/discord-script.py
def ToEpoch(Time: tuple[int, int]):
    DST = (Time[1] == 1) if Time[1] else False

    DeltaTime = (datetime.now().astimezone() if DST else datetime.now(timezone.utc))

    return Time[0] - int(DeltaTime.utcoffset().total_seconds()) # type: ignore